...
exports.playOnDevice = async function playOnDevice(deviceName, play) {
  const token = await getToken();
...
exports.setVolume = async function setVolume(volume, deviceName) {
  ...
  if (typeof deviceName !== 'undefined' && deviceName) {
    devicsFuture = listDevices(token);
  }
  const devices = await devicesFuture;
...
exports.adjustVolume = async function setVolume(volumeChange, deviceName) {
  ...
  return setVolumeTo(token, newVolume, device);
